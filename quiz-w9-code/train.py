...
# Perform the upsampling
upsample_filter_np_x2 = bilinear_upsample_weights(2,  # upsample_factor,#2x上采样的双线性插值filter权重
                                                  number_of_classes)

upsample_filter_tensor_x2 = tf.Variable(upsample_filter_np_x2, name='vgg_16/fc8/t_conv_x2')
#对vgg_16最后输出的logits,执行2x的上采样，得到的结果是和aux_logits_16s一样大小，为32x22x21
upsampled_logits = tf.nn.conv2d_transpose(logits, upsample_filter_tensor_x2,
                                          output_shape=tf.shape(aux_logits_16s),
                                          strides=[1, 2, 2, 1],
                                          padding='SAME')

#将两个同维度的feature maps做element wise相加，结果还是28x28x21，放入upsampled_logits
upsampled_logits = upsampled_logits + aux_logits_16s
#再从vgg_16中的endpoints['vgg_16/pool3']中取出feature maps,然后再通过一个1x1的卷积对feature maps做
#一个21分类处理，初始化用的是zeros_initializer,所以输出的结果不会有任何改变,赋值到aux_logits_8s，结果是64x44x21
pool3_feature = end_points['vgg_16/pool3']
with tf.variable_scope('vgg_16/fc8'):
    aux_logits_8s = slim.conv2d(pool3_feature, number_of_classes, [1, 1],
                             activation_fn=None,
...
#对upsampled_logits再做一次2x的上采样，得到的结果和aux_logits_8s一样大小，为64x44x21
upsampled_logits = tf.nn.conv2d_transpose(upsampled_logits, upsample_filter_tensor_x2,
                                 output_shape=tf.shape(aux_logits_8s),
                                 strides=[1, 2, 2, 1],
                                         padding='SAME')

#将两个同维度的feature maps做element wise相加，结果还是64x44x21，仍然赋值回upsampled_logits
upsampled_logits = upsampled_logits + aux_logits_8s
